<?php
/*Plugin Name:Social Login for Wordpress in french language francais  
...
$id=$userprofile->ID;
$Provider=$userprofile->Provider;
$msg="<p>S'il vous plait, entrez votre adresse e-mail a` aller plus loin</p>";
...
$user_email=urldecode($_POST['email']);
   if (! is_email($user_email) OR email_exists ($user_email))
    {
		 $msg="<p style='color:red;'>Cet email est de`ja` enregistre` ou non valide, s'il vous plait choisir un autre.</p>";
		 $id=$_POST['Id'];  
		 $Fname=$_POST['fname'];
		 $Lname=$_POST['lname'];
...
<input type="submit" id="LoginRadiusRedSliderClick" name="LoginRadiusRedSliderClick" value="Envoyer" class="inputbutton">
<input type="submit" value="Annuler" class="inputbutton" onClick="history.back()" />
